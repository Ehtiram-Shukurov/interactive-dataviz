        ...
        {#each movies as movie}
            {#if x == "genres"}
                {#each movie["genres"] as genre}
                    ...
                    <circle
                        cx={xScale ? xScale(genre as any) : usableArea.left}
                        cy={yScale ? yScale(movie[y] as any) : usableArea.bottom}
                        r={sizeScale && size ? (sizeScale(movie[size] as any) ?? sizeRange[0]) : sizeRange[0]}
            ...
            {:else}
                ...
                <!-- svelte-ignore a11y_no_static_element_interactions -->
                <circle
                    cx={xScale ? xScale(movie[x] as any) : usableArea.left}
                    cy={yScale ? yScale(movie[y] as any) : usableArea.bottom}
                    r={sizeScale && size ? (sizeScale(movie[size] as any) ?? sizeRange[0]) : sizeRange[0]}
